...
def do_deploy(archive_path):
    ...
    try:
        ...
        run('rm -rf  /data/web_static/releases/{}/web_static/'.format(archive_path[9:-4]))

        run('rm -rf /data/web_static/current')
        run('ln -s /data/web_static/releases/{}/ /data/web_static/current'.format(archive_path[9:-4]))
